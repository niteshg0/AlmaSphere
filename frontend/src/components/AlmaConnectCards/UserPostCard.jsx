    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className="font-bold text-gray-200">{post.username}</h3>
            <p className="text-sm text-gray-400">{post.year} ({post.course})</p>
          </div>
        </div>
        <motion.button
      ...
      {post.hasMedia && (
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="aspect-video bg-gray-700/30 rounded-2xl border border-gray-600/40 flex items-center justify-center">
          ...
          <div className="space-y-2">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-8 bg-gray-700/30 rounded-lg flex items-center px-3">
                <div className="w-6 h-6 rounded-full bg-gray-600 mr-2" />
                <span className="text-sm text-gray-400">Comment {i + 1}</span>
              </div>
            ))}
          </div>
        </div>
